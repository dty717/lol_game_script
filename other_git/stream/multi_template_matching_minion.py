...
for (startX, startY, endX, endY) in pick:
  ...
  # print(centerColor)
  HP_Length = 59
  if centerColor[0] >100 and  centerColor[1] > 85 and centerColor[2] <90:
    for i in range(startX+1,endX+59):
      _color = image[int((startY+endY)/2),i]
      # print(_color)
      if _color[0] >100 and  _color[1] > 85 and _color[2] <90:
        pass
      else:
        HP_Length = i - startX
        break
    print("friend",HP_Length)
  elif centerColor[0] < 100 and centerColor[0] > 50 and centerColor[1] < 95 and centerColor[1] > 50 and centerColor[2] > 90:
    for i in range(startX+1,endX+59):
      _color = image[int((startY+endY)/2),i]
      # print(_color)
      if _color[0] <100 and  _color[1] < 95 and _color[2] >90:
        pass
      else:
        HP_Length = i - startX
        break
    print("enemy",HP_Length)
  else:
    continue
    # break
  ...
  # else:
    # cv2.imwrite('heroName'+str(startX)+".jpg", _image)
  cv2.rectangle(image, (startX, startY), (startX+HP_Length, endY),
    (255, 0, 0), 1)
...
# python multi_template_matching_minion.py --image images/g0/3a0z2sVYl4.jpg --template images/minion.jpg --threshold 0.986